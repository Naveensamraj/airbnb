...
import { Building2, BookOpen, Euro, ArrowRightLeft, CalendarCheck, CalendarX, Clock } from 'lucide-react';
...
export default function AdminDashboard() {
  const { bookings, properties, payments, notifications, revenueData, occupancyData, bookingTrendData } = useData();
  ...
  const metrics = useMemo(() => {
    ...
    const activeProperties = properties.filter(p => p.status !== 'maintenance').length;
    return { todayCheckins, todayCheckouts, pending, pendingPayments, totalRevenue, activeProperties };
  }, [bookings, payments, properties, revenueData]);

  const revenueChartData = useMemo(() => (
    revenueData.map(d => ({ label: d.month, value: d.revenue, value2: d.expenses }))
  ), [revenueData]);

  const occupancyChartData = useMemo(() => (
    occupancyData.map(d => ({ label: d.property, value: d.rate }))
  ), [occupancyData]);

  const trendChartData = useMemo(() => (
    bookingTrendData.map(d => ({ label: d.week, value: d.bookings }))
  ), [bookingTrendData]);

  const recentBookings = useMemo(() => bookings.slice(0, 5), [bookings]);

  const recentActivity = useMemo(() => (
    notifications.map(n => ({ ...n, time: new Date(n.created_at).toLocaleDateString() })).slice(0, 5)
  ), [notifications]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Monthly Revenue" value={metrics.totalRevenue} icon={Euro} trend={18} trendLabel="vs last month" color="amber" prefix="€" />
        <StatCard title="Pending Approvals" value={metrics.pending} icon={Clock} color="red" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Active Properties" value={metrics.activeProperties} icon={Building2} color="blue" />
        <StatCard title="Today's Check-ins" value={metrics.todayCheckins} icon={CalendarCheck} color="emerald" />
        <StatCard title="Today's Check-outs" value={metrics.todayCheckouts} icon={CalendarX} color="amber" />
        <StatCard title="Pending Payments" value={metrics.pendingPayments} icon={ArrowRightLeft} color="red" />
      </div>
