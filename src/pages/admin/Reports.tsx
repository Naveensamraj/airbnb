...
export default function Reports() {
  const { bookings, properties, revenueData, occupancyData, bookingTrendData } = useData();
  ...
  const reportMetrics = useMemo(() => {
    ...
    const avgOccupancy = occupancyData.length
      ? Math.round(occupancyData.reduce((s, o) => s + o.rate, 0) / occupancyData.length)
      : 0;
    const cancellationRate = bookings.length
      ? `${Math.round((cancelledCount / bookings.length) * 100)}%`
      : '0%';
    return { cancelledCount, completedCount, totalRevenue, avgOccupancy, cancellationRate };
  }, [bookings, revenueData, occupancyData]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Revenue YTD', value: `${CURRENCY}${reportMetrics.totalRevenue.toLocaleString()}`, color: 'text-emerald-700 bg-emerald-50' },
          { label: 'Avg. Occupancy', value: `${reportMetrics.avgOccupancy}%`, color: 'text-blue-700 bg-blue-50' },
          { label: 'Completed Stays', value: reportMetrics.completedCount, color: 'text-slate-700 bg-slate-100' },
          { label: 'Cancellation Rate', value: reportMetrics.cancellationRate, color: 'text-red-700 bg-red-50' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          ...
          <BarChartComponent
            data={bookingTrendData.map(d => ({ label: d.week, value: d.bookings }))}
            color="#2563eb"
            height={140}
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <HorizontalBar
            data={occupancyData.map(d => ({ label: d.property, value: d.rate }))}
            color="#2563eb"
            suffix="%"
          />
        </div>
        ...
        <div className="card p-5">
          ...
          {[
            ...
            { label: 'Cancelled', count: reportMetrics.cancelledCount, color: 'bg-red-500' },
            ...
            <div key={label} className="flex items-center gap-3 mb-3">
              ...
              <div className="w-24 h-1.5 bg-slate-100 rounded-full">
                <div className={`h-full rounded-full ${color}`} style={{ width: `${bookings.length ? (count / bookings.length) * 100 : 0}%` }} />
              </div>
            </div>
          ))}
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100">
              {properties.filter(h => h.is_approved).map((h, i) => {
                ...
                const occ = occupancyData.find(o => o.property.includes(h.name.split(' ')[0]))?.rate ?? 0;
