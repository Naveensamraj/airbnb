...
import { Euro, TrendingUp, TrendingDown, Download, CreditCard, Plus, Trash2 } from 'lucide-react';
...
export default function Finance() {
  const { payments, expenses, addExpense, deleteExpense, deletePayment, revenueData } = useData();
              ...
              {filteredPayments.map(p => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="table-td">
                    <Badge label={p.type} variant={(TYPE_COLORS[p.type] as 'default' | 'success' | 'warning' | 'error' | 'info' | 'purple') ?? 'default'} />
                  </td>
                  ...
                  <td className="table-td">
                    <button onClick={() => deletePayment(p.id)}
                      className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition-colors" title="Delete payment">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select className="input" value={expenseForm.category}
                onChange={e => setExpenseForm(f => ({ ...f, category: e.target.value as typeof f.category }))}>
