      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                    ...
                    {notifications.map(n => (
                      <div
                        ...
                        className={`px-4 py-3 flex items-start justify-between gap-2 ${
                          n.is_read ? 'opacity-60' : 'bg-blue-50/30 cursor-pointer'
                        }`}
                      >
                        <div className="flex items-start gap-2 min-w-0 flex-1">
                          <span className={`mt-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded flex-shrink-0 ${typeColor(n.type)}`}>
                            {n.type}
                          </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => logout()}
              className="p-2 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-red-600 transition-colors ml-1"
              title="Sign Out"
            >
              <LogOut size={18} />
            </button>
