            ...
            <g key={i}>
              ...
              {hasSecond && d.value2 !== undefined && (
                <rect x={x2} y={padY + chartH - (d.value2 / max) * chartH} width={barW} height={(d.value2 / max) * chartH}
                  ...
                  <title>{`${d.label} (2): ${prefix}${d.value2}`}</title>
                </rect>
              )}
...
export function LineChart({ data, height = 120, color = '#2563eb', prefix = '' }: LineChartProps) {
  ...
  const pts = data.map((d, i) => {
    const x = padX + (i / denominator) * W;
    const y = padY + H - ((d.value - min) / range) * H;
    return { x, y, d };
  });

  const linePath = pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ');
  const areaPath = pts.length ? `${linePath} L${pts[pts.length - 1].x},${padY + H} L${pts[0].x},${padY + H} Z` : '';
      ...
      {pts.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r="3" fill={color}>
            <title>{`${p.d.label}: ${prefix}${p.d.value}`}</title>
          </circle>
...
export function HorizontalBar({ data, color = '#2563eb', suffix = '%' }: HorizontalBarProps) {
  if (!data || data.length === 0) {
    return <div className="text-xs text-slate-400 py-4 text-center">No data available</div>;
  }
